...
TemperatureSensorWatcher::TemperatureSensorWatcher(AllJoynBusAttachment^ busAttachment) :
...
TemperatureSensorWatcher::~TemperatureSensorWatcher()
{
    UnregisterFromBus();
}

void TemperatureSensorWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.OpenT2T.Sample.SuperPopular.TemperatureSensor" };
...
void TemperatureSensorWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.OpenT2T.Sample.SuperPopular.TemperatureSensor"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.OpenT2T.Sample.SuperPopular.TemperatureSensor"),
            port);
        Added(this, args);
    }
...
void TemperatureSensorWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<TemperatureSensorWatcher>
        };
...
void TemperatureSensorWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_TemperatureSensorIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &TemperatureSensorWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void TemperatureSensorWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void TemperatureSensorWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
