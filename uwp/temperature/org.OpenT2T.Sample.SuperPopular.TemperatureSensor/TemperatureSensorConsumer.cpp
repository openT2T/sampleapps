...
std::map<alljoyn_interfacedescription, WeakReference*> TemperatureSensorConsumer::SourceInterfaces;

TemperatureSensorConsumer::TemperatureSensorConsumer(AllJoynBusAttachment^ busAttachment)
...
TemperatureSensorConsumer::~TemperatureSensorConsumer()
{
...
void TemperatureSensorConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void TemperatureSensorConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void TemperatureSensorConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus TemperatureSensorConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<TemperatureSensorJoinSessionResult^>^ TemperatureSensorConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ TemperatureSensorWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> TemperatureSensorJoinSessionResult^
    {
        auto result = ref new TemperatureSensorJoinSessionResult();
        result->Consumer = ref new TemperatureSensorConsumer(watcher->BusAttachment);
        result->Status = result->Consumer->JoinSession(serviceInfo);
        return result;
    });
}

IAsyncOperation<TemperatureSensorGetCurrentTemperatureResult^>^ TemperatureSensorConsumer::GetCurrentTemperatureAsync()
{
    return create_async([this]() -> TemperatureSensorGetCurrentTemperatureResult^
    {
...
IAsyncOperation<TemperatureSensorGetTemperatureTrendResult^>^ TemperatureSensorConsumer::GetTemperatureTrendAsync()
{
    return create_async([this]() -> TemperatureSensorGetTemperatureTrendResult^
    {
...
void TemperatureSensorConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
...
void TemperatureSensorConsumer::CallErrorSignalHandler(_In_ const alljoyn_interfacedescription_member* member, _In_ alljoyn_message message)
{
    ...
    auto consumer = source->second->Resolve<TemperatureSensorConsumer>();
...
int32 TemperatureSensorConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<TemperatureSensorConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<TemperatureSensorConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<TemperatureSensorConsumer>
    };
    ...
    alljoyn_interfacedescription description = alljoyn_busattachment_getinterface(m_nativeBusAttachment, "org.OpenT2T.Sample.SuperPopular.TemperatureSensor");
