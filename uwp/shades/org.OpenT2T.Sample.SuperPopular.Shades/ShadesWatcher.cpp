...
ShadesWatcher::ShadesWatcher(AllJoynBusAttachment^ busAttachment) :
...
ShadesWatcher::~ShadesWatcher()
{
    UnregisterFromBus();
}

void ShadesWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.OpenT2T.Sample.SuperPopular.Shades" };
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.OpenT2T.Sample.SuperPopular.Shades"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.OpenT2T.Sample.SuperPopular.Shades"),
            port);
        Added(this, args);
    }
...
void ShadesWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<ShadesWatcher>
        };
...
void ShadesWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_ShadesIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &ShadesWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void ShadesWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void ShadesWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
