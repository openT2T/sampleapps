...
std::map<alljoyn_busobject, WeakReference*> ShadesProducer::SourceObjects;
std::map<alljoyn_interfacedescription, WeakReference*> ShadesProducer::SourceInterfaces;

ShadesProducer::ShadesProducer(AllJoynBusAttachment^ busAttachment)
...
ShadesProducer::~ShadesProducer()
{
    UnregisterFromBus();
    delete m_weak;
}

void ShadesProducer::UnregisterFromBus()
{
...
bool ShadesProducer::OnAcceptSessionJoiner(_In_ alljoyn_sessionport sessionPort, _In_ PCSTR joiner, _In_ const alljoyn_sessionopts opts)
{
...
void ShadesProducer::OnSessionJoined(_In_ alljoyn_sessionport sessionPort, _In_ alljoyn_sessionid id, _In_ PCSTR joiner)
{
    ...
    {
        AllJoynHelpers::SessionLostHandler<ShadesProducer>,
        AllJoynHelpers::SessionMemberAddedHandler<ShadesProducer>,
        AllJoynHelpers::SessionMemberRemovedHandler<ShadesProducer>
    };
...
void ShadesProducer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void ShadesProducer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void ShadesProducer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void ShadesProducer::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {   
        QStatus result = AllJoynHelpers::CreateProducerSession<ShadesProducer>(m_busAttachment, m_weak);
        if (ER_OK != result)
        {
            StopInternal(result);
            return;
...
void ShadesProducer::CallOpenHandler(_Inout_ alljoyn_busobject busObject, _In_ alljoyn_message message)
{
    ...
    ShadesProducer^ producer = source->second->Resolve<ShadesProducer>();
    if (producer->Service != nullptr)
    {
        ...
        ShadesOpenResult^ result = create_task(producer->Service->OpenAsync(callInfo)).get();
        create_task([](){}).then([=]
        {
            int32 status;
...
void ShadesProducer::CallCloseHandler(_Inout_ alljoyn_busobject busObject, _In_ alljoyn_message message)
{
    auto source = SourceObjects.find(busObject);
    if (source == SourceObjects.end())
    {
    ...
    if (producer->Service != nullptr)
    {
        ...
        ShadesCloseResult^ result = create_task(producer->Service->CloseAsync(callInfo)).get();
        create_task([](){}).then([=]
        {
            int32 status;
...
void ShadesProducer::CallErrorSignalHandler(_In_ const alljoyn_interfacedescription_member* member, _In_ alljoyn_message message)
{
    ...
    auto producer = source->second->Resolve<ShadesProducer>();
...
QStatus ShadesProducer::AddMethodHandler(_In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_methodhandler_ptr handler)
{
...
QStatus ShadesProducer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
    alljoyn_interfacedescription_member member;
    if (!alljoyn_interfacedescription_getmember(interfaceDescription, methodName, &member))
    {
...
QStatus ShadesProducer::OnPropertyGet(_In_ PCSTR interfaceName, _In_ PCSTR propertyName, _Inout_ alljoyn_msgarg value)
{
...
QStatus ShadesProducer::OnPropertySet(_In_ PCSTR interfaceName, _In_ PCSTR propertyName, _In_ alljoyn_msgarg value)
{
    UNREFERENCED_PARAMETER(propertyName);
    UNREFERENCED_PARAMETER(value);
    UNREFERENCED_PARAMETER(interfaceName);
...
void ShadesProducer::Start()
{
    ...
    QStatus result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_ShadesIntrospectionXml);
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    result = AllJoynHelpers::CreateBusObject<ShadesProducer>(m_weak);
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    alljoyn_interfacedescription interfaceDescription = alljoyn_busattachment_getinterface(AllJoynHelpers::GetInternalBusAttachment(m_busAttachment), "org.OpenT2T.Sample.SuperPopular.Shades");
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^,AllJoynBusAttachmentStateChangedEventArgs^>(this, &ShadesProducer::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void ShadesProducer::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void ShadesProducer::StopInternal(int32 status)
{
    UnregisterFromBus();
    Stopped(this, ref new AllJoynProducerStoppedEventArgs(status));
}

int32 ShadesProducer::RemoveMemberFromSession(_In_ String^ uniqueName)
{
...
PCSTR org::OpenT2T::Sample::SuperPopular::Shades::c_ShadesIntrospectionXml = "<interface name=\"org.OpenT2T.Sample.SuperPopular.Shades\">"
"  <!-- open/raise the window treatment -->"
"  <method name=\"open\" />"
"  <!-- close/lower the window treatment -->"
"  <method name=\"close\" />"
